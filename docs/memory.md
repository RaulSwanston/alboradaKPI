...
- **Visibilidad por rol centralizada en `RenderView.anima` (PASO 4):** al final de cada render (carga global y parcial) se aplica `classList.toggle('hidden', !permissions.isAdmin | !permissions.isResident)` a todos los `[data-role-required]`. Antes solo lo hacía `initSessionUI()` desde el topbar, que en carga global corría antes de inyectar los módulos y fallaba (el botón "+ nueva transacción" quedaba visible para residentes). Convención: poner `data-role-required="admin"` (o `"resident"`) en el elemento; la fuente de verdad es el custom claim (`contexto.data.permissions`), no el campo `role` de Firestore.
- **CSS `.hidden` vs `display` de elementos:** la clase `.hidden { display:none }` es vencida por reglas de igual especificidad definidas después (ej. `.btn-new-transaction { display:flex }`, `.modal-overlay { display:flex }`). Siempre crear la regla específica `.elemento.hidden { display:none }` (especificidad 0,2,0) o usar `!important` en `.hidden`. Aplica a cualquier control oculto por rol.
- **Identidad/Rol = fuente única en `sessionGuard` (`middleware/auth.js`):** el rol admin se deriva del custom claim (`isAdmin = !!idTokenResult.claims.admin`) y se propoga a `contexto.data.permissions.role` y `contexto.data.userProfile.role` (fallback al doc). NUNCA usar `userProfile.role` (crudo de Firestore) para decidir admin: un admin por claim cuyo doc no tenga `role:'admin'` se clasificaba como no-admin. Los controllers deben consumir `contexto.data.userProfile` (ya claim-aware) en vez de re-hacer `User.getById`. `financialSummary`, `quickActions`, `navigator` ya lo usan y se corrigen solos.
