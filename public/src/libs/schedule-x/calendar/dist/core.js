...
function ViewSelection() {
    ...
    return (jsxs("div", { className: `sx__view-selection ${isOpen ? 'is-open' : ''}`, children: [jsx("label", { for: viewSelectId, id: viewLabelId, className: "sx__view-selection-label", children: $app.translate('View') }), jsxs("button", { id: viewSelectId, type: "button", "aria-describedby": viewLabelId, "aria-label": $app.translate('Select View'), className: "sx__view-selection-selected-item sx__ripple", onClick: () => setIsOpen(!isOpen), onKeyDown: handleSelectedViewKeyDown, children: [selectedViewLabel, jsx("img", { className: "sx__view-selection-chevron", src: img, alt: "" })] }), isOpen && (jsx("ul", { "data-testid": "view-selection-items", className: "sx__view-selection-items", children: availableViews.map((view) => (jsx("li", { children: jsx("button", { type: "button", "aria-label": $app.translate('Select View') +
                            ' ' +
...
function TimeAxis() {
    const $app = useContext(AppContext);
    const [gridSteps, setGridSteps] = useState(() => computeGridSteps($app.config.dayBoundaries.value, $app.config.isHybridDay, $app.config.weekOptions.value.gridStep));
    useSignalEffect(() => {
